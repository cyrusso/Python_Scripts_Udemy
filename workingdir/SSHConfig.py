...
def user_is_valid():
    ...
    while True:
        ...
        #Changing output message
        if os.path.isfile(user_file) == True:
            print "\n* Usernaem\password file has been validated. Waiting for command file...\n"
            break
        else:
            print "\n* Files %s does not exist! Please check and try again!\n" % user_file
            continue
...
#Open SSHv2 connectiont o devices

def open_ssh_conn(ip):
    print "Starting the thread"
    time.sleep(5)
    print "Ending the thread"
    #Change exceptin message
    try:
        ...
        #Starting form the beginning of the file
        selected_user_file.seek(0)
        ...
        if re.search(r"% Invalid input detected at", router_output):
            print "* There was at least one IOS sysntax error on device %s" % ip
        else:
