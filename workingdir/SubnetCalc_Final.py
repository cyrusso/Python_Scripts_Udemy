...
def subnet_calc():
    try:
        ...
        while True:
            ...
            a = ip_address.split('.')
            #print a
            if(len(a) == 4) and (1 <= int(a[0]) <= 223) and (a[0] != 127) and (int(a[0]) != 169 or int(a[1]) != 254) and (0 <= int(a[1]) <= 255 and 0 <= int(a[2]) <= 255 and 0 <= int(a[3]) <= 255):
                break
            else:
        ...
        #Algorithm for subnet identificaiton, based on IP and Subnet Masks

        #Convert mask to binary string
        mask_octets_padded = []
        mask_octests_decimal = subnet_mask.split('.')
        #print mask_octests_decimal
        for octet_index in range(0, len(mask_octests_decimal)):
            #print bin(int(mask_octests_decimal[octet_index]))
            binary_octet = bin(int(mask_octests_decimal[octet_index])).split('b')[1]
            #print binary_octet
        ...
        #Counting host bits in the mask and calculating number of hosts/subnet_calc
        ...
        wildcard_octets = []
        for w_octet in mask_octests_decimal:
        ...
        #print binary_ip
        #Obtain the network address broadcast address from the binary strings obtained above
        ...
        while True:
            ...
            if generate == "y":
                ...
                #Obtain available IP address in range based on the difference between octets in broadcast and network address
                ...
                #print y_iaddr
                print "Random IP addres is: %s" % y_iaddr
                print "\n"
                continue
            else:
                print "Ok, bye!\n"
